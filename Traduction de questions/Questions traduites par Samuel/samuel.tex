...
4026
-- Which function  correctly describes the plot below?\\
\begin{center}
...
4028-- The president of a large company wants to build a phone chain
to join his $10,000$ employees. He calls $5$ employees and asks them
to call $5$ more employees each and so on. To better visualize the
chain, he wants find an equation that describes the relationship
between $n$, the \emph{group number} of employees down in the chain,
and the total number of people reached when the phone chains gets to
their level. The first group is made up of the president only and
the $5$ employees
he is joining are the second group, and so on.\\

True or false?\\
The function $f\left(n\right)=10,000(5)^{n} $ correctly describes this situation.\\

R\'eponse : False\\

R\'etroaction :\\

We want to find the equation that correctly describes this
situation. The president joins $5$ employees who join $5$ people
each, and so on. The table below summarizes this situation: \\
\begin{center}
\begin{tabular}{ |l|l|}
\hline
Group Number & Total No. of People Reached\\
\hline
1 & 5\\
2 & 25\\
3 & 125\\
...
times larger. The function that rules this kind of behavior is a
base 5 exponential. More precisely:
\begin{center}
$f\left(n\right)=5^{n}$
\end{center}
...
4034-- In the spring, I empty my pool with my four cubic meter flow
rate pump. The job takes an hour and then I fill it with clean water
with the same pump. The pool contains 54 cubic meter of water when
it is full and is two-thirds full before being emptied in the
spring. How long will it take the pump to empty the pool and how
long will it take to fill it up?\\
...
Since it contains 54 cubic meters when full and is filled to the two
thirds of its capacity, it holds $\frac{2}{3}$ of $54 = 36 $ cubic
meters of water before being emptied. With a $4$ cubic meter
flow rate, the time needed to empty the pool is:\\[2mm]
$\frac{36}{4}=9$\\

The pool is empty after 9 hours of pumping. The time needed to fill it to full capacity is :\\[2mm]
$\frac{54}{4}=13,5$\\
