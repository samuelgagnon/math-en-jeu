...
class ImageController extends Zend_Controller_Action {
	...
	function deleteAction() {
		$session = new Zend_Session_Namespace('deleteImage');
		$this->view->action = "delete";
		$session->id = Zend_Filter::get($this->_request->getParam('id'), 'Digits');
		$this->view->id = $session->id;
		$this->view->step = 1;

		if ($this->_request->getPost('step') == 1) {
			$session->id = Zend_Filter::get($this->_request->getParam('id'), 'Digits');
			$this->view->id = $session->id;
			$this->view->message = "Click finish to remove picture";
			$session->choice = $this->_request->getPost('choice');
			$this->view->choice = $session->choice;
			$this->view->step = 2;

		} elseif ($this->_request->getPost('step') == 2) {
			if (($session->id>0)&&($session->choice=="Yes")) {
				$image = new Image();
				$row = $image->fetchRow('image_id =' . $session->id);
				$where = $image->getAdapter()->quoteInto('image_id = ?', $session->id);

				//delete the file
				$registry = Zend_Registry::getInstance();
				$config = $registry->get('config');
				@unlink($config->file->image->dir . DIRECTORY_SEPARATOR . $row->physical_name);
				@unlink($config->file->image->html->dir . DIRECTORY_SEPARATOR . str_replace(".eps",".jpeg",$row->physical_name));
		...
		if ($this->_request->isPost()) {
			if (is_uploaded_file($_FILES["file"]["tmp_name"])) {
              ...
              'user_id' => Zend_Auth::getInstance()->getIdentity()->user_id
					...
					$this->view->message = "Could not upload the file, check that the file is a valid picture.";
				}
			}
		}
