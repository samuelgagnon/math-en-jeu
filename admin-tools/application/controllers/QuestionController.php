...
class QuestionController extends Zend_Controller_Action {
	...
	function init() {
		...
		Zend_Loader::loadClass('QuestionForm', '/question');
	...
	function editAction()
	{
		...
		$this->view->question_id = $rowQ->question_id;
		$this->view->answer_type_id = $rowQ->answer_type_id;
		$this->view->category_id = $rowQ->category_id;
		$this->view->subject_id = $rowQ->subject_id;

		$questionInfo = new QuestionInfo();
		$rowQI = $questionInfo->fetchRow('question_id=' . $question_id . ' and language_id=' . $language_id);
		$this->view->question_latex = $question_latex = $rowQI->question_latex;
		$this->view->feedback_latex = $feedback_latex = $rowQI->feedback_latex;
		$this->view->answer_a_latex = $answer_a_latex = $rowQI->answer_a_latex;
		$this->view->answer_b_latex = $answer_b_latex = $rowQI->answer_b_latex;
		$this->view->answer_c_latex = $answer_c_latex = $rowQI->answer_c_latex;
		$this->view->answer_d_latex = $answer_d_latex = $rowQI->answer_d_latex;
		$this->view->good_answer = $good_answer = $rowQI->good_answer;
		...
		$this->view->answer_type_name = $answer_type_name = utf8_encode($rowAT->name);

		$categoryInfo = new CategoryInfo();
		$rowCI = $categoryInfo->fetchRow('category_id=' . $rowQ->category_id . ' and language_id=' . $language_id);
		$this->view->category_name = $category_name = utf8_encode($rowCI->name);

		$subjectInfo = new SubjectInfo();
		$rowSI = $subjectInfo->fetchRow('subject_id=' . $rowQ->subject_id . ' and language_id=' . $language_id);
		$this->view->subject_name = $subject_name = utf8_encode($rowSI->name);

		$questionLevel = new QuestionLevel();
		$this->view->levels = $questionLevel->fetchAll('level_id in (select level_id from level where language_id=' .  $language_id . ")" . " and question_id = " . $question_id);
		$rowQL = $questionLevel->fetchRow('question_id='.$question_id);


		$form = new QuestionForm(null, 'francais', $subject_name, $answer_type_name, $category_name, $question_latex, $feedback_latex, $answer_a_latex, $answer_b_latex, $answer_c_latex, $answer_d_latex, $good_answer);
		...
		if (!$this->_request->isPost()) {
			...
			if ($form->isValid($formData)) {
				...
				if ($question_id > 0 && $language_id > 0) {
					...
					$rowAT->answer_type_name = $form->getValue('answer_type_name');
					$rowAT->answer_type_tag = $rowAT->findParentAnswerType()->tag;
					$rowAT->save();

					//assign category
					$rowCI->category_name = $form->getValue('category_name');
					$rowCI->save();

					//assign subject
					$rowSI->subject_name = $form->getValue('subject_name');
					$rowSI->save();

					//assign level
					while ($rowQL != null) {
						$rowQL->level_id = $form->getValue('level_id');
						$rowQL->value = $form->getValue('value');
						$rowQL = $questionLevel->fetchRow('question_id='.$question_id);
						$rowQL->save();
					}

					//assign question
					$rowQ->subject_id = $subjectInfo->fetchRow('subject_name=' . $rowSI->subject_name . ' and language_id=' .  $language_id);
					$rowQ->category_id = $categoryInfo->fetchRow('category_name=' . $rowCI->category_name . ' and language_id=' .  $language_id);
					$rowQ->answer_type_id = $answerTypeInfo->fetchRow('answer_type_name=' . $rowAT->answer_type_name . ' and  language_id=' . $language_id);
					$rowQ->save();

					//assign question info
					$rowQI->question_latex = $form->getValue('question_latex');
					$rowQI->answer_a_latex = $form->getValue('answer_a_latex');
					$rowQI->answer_b_latex = $form->getValue('answer_b_latex');
					$rowQI->answer_c_latex = $form->getValue('answer_c_latex');
					$rowQI->answer_d_latex = $form->getValue('answer_d_latex');
					$rowQI->good_answer = $form->getValue('good_answer');
					$rowQI->question_flash_file = "Q-" . $question_id . "-" . $short_name . ".swf";
					$rowQI->feedback_flash_file = "F-" . $question_id . "-" . $short_name . ".swf";
					$rowQI->feedback_latex = $form->getValue('feedback_latex');
					$rowQI->save();
			...
			$form = new QuestionForm(null, 'français');
			$question_id = Zend_Filter::get($this->_request->getParam('question_id'), 'Digits');
			$language_id = Zend_Filter::get($this->_request->getParam('language_id'), 'Digits');
			if ($question_id > 0 && $language_id > 0) {
				$question = new Question();
				...
				$questionInfo = new QuestionInfo();
				$rowQI = $questionInfo->fetchRow('question_id=' . $question_id . ' and language_id=' . $language_id);
				$form->populate($questionInfo->toArray());
			}	//if end
		}	//isPost-else end
	}	//function end
