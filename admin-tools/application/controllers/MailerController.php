...
class MailerController extends Zend_Controller_Action {
  ...
  function deleteAction() {
    $session = new Zend_Session_Namespace('deleteMail');
		...
		if ($this->_request->getPost('step') == 1) {
			$session->id = Zend_Filter::get($this->_request->getParam('id'), 'Digits');
			$this->view->id = $session->id;
			$this->view->message = "Click finish to delete mail";
			...
			if (($session->id>0)&&($session->choice=="Yes")) {
				$mailTemplate = new MailTemplate();
        $where = $mailTemplate->getAdapter()->quoteInto('mail_template_id = ?', $session->id);
