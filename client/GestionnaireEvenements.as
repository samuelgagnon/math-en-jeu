...
class GestionnaireEvenements
{
    ...
    private var listeDesPersonnages:Array;   // liste associant les idPersonnage avec les nomUtilisateurs dans la table où on ait
    private var motDePasse:String;  // notre mot de passe pour pouvoir jouer
    private var nomSalle:String;  //  nom de la salle dans laquelle on ait
    private var numeroTable:Number;   //   numéro de la table dans laquelle on ait
    private var tempsPartie:Number;   //  temps que va durée la partie, en minutes
    private var idPersonnage:Number;   //  le idPersonnage que nous avons choisi (le dessin)
    private var motDePasseSalle:String;   // le mot de passe de la salle dans laquelle on ait
    private var listeDesJoueursDansSalle:Array;  // liste des joueurs dans la salle qu'on ait. Un joueur contient un nom (nom)
    private var listeDesSalles:Array;    //  liste de toutes les salles
	private var listeChansons:Array;    //  liste de toutes les chansons
    private var listeDesJoueursConnectes:Array;   // la première liste qu'on recoit, tous les joueurs dans toutes les salles. Un joueur contient un nom (nom)
    //  liste de toutes les tables dans la salle où on ait
...
return this.objGestionnaireCommunication;
    ...
    {
    // c'est la fonction qui va Ítre appellee lorsque le GestionnaireCommunication aura
        // reçu la reponse du serveur
        // objetEvenement est un objet qui est propre  chaque fonction comme retourConnexion
        // (en termes plus informatiques, on appelle Áa un eventHandler -> fonction qui gËre
			 ...
			 for(var i:Number = 0; i < 3; i++)
                {
					...
					if(objetEvenement.listePersonnageJoueurs[i].idPersonnage == 0)
					{
						//movClip._y = 350;
						    movClip._y = 150 + i*60;
					}
	...
	public function retourDemarrerMaintenant(objetEvenement:Object)
     {
        ...
        trace("*********************************************");
        trace("debut de retourDemarrerMaintenant   "+objetEvenement.resultat);
    ...
    ////////////////////////////////////////////////////////////////////////////////////////////////////
    // Ça c'est la fonction qui va être appellée lorsque le GestionnaireCommunication aura
      ...
      switch(objetEvenement.resultat)
         {
			...
			{
				_level0.loader.contentHolder.planche.obtenirPerso().ajouterImageBanque(_level0.loader.contentHolder.planche.obtenirPerso().obtenirNombreObjet(), objetEvenement.argent.type,  objetEvenement.argent.id, 75);
			}
      ...
      switch(objetEvenement.resultat)
         {
			...
			switch(objetEvenement.objetUtilise.typeObjet)
			{
				...
				// le serveur envoit une mauvaise réponse
...
//todo corriger la ligne précédente si problème
      ...
      switch(objetEvenement.resultat)
         {
             ...
             case "Deplacement":
		     	if(objetEvenement.deplacementAccepte)
		     	{
					...
					if(objetEvenement.collision == "minigame")
					{
					}
					
					if(objetEvenement.collision == "magasin")
					{
					}
    ...
    if(indice != -1)
    {
        ...
        // enlever la table de la lise si elle est pleine
    ...
    if(objetEvenement.noTable == this.numeroTable)
    {
        for(i = 0; i < 3; i++)
        {
            //  on enleve le nom du joueur dans la liste et à l'écran
        ...
        for(i = 0; i < objetEvenement.positionJoueurs.length; i++)
        {
            for(j = 0; j < 4; j++)
            {
                if(this.listeDesPersonnages[j].nom == objetEvenement.positionJoueurs[i].nom)
                {
		    		...
		    		trace("Construction du personnage : " + this.listeDesPersonnages[j].nom + " " + objetEvenement.positionJoueurs[i].x + " " + objetEvenement.positionJoueurs[i].y + " " + i + " " + this.listeDesPersonnages[j].id);
				}
            }
        }
    ...
    trace("fin de evenementPartieTermineee    ");
    trace("*********************************************\n");
    }
