...
public class SmacParser implements SmacUI
{
		...
		try
		{
			...
			while (line != null)
			{
				...
				if (nextRegion == Tag.Begin)
				{
					...
					questionInfo.put(Tag.PATH, filename);
	...
	private String getQdiff(Map<Tag,String> q1)
	{
		String diff = q1.get(Tag.Qdifficulty);
		if (diff != null) return diff;
		diff = q1.get(Tag.Rdifficulty);
		return rdiffToQdiffMap.get(Integer.parseInt(diff));
	}

	public int numParsed()
	{
		return allParsedQuestions == null? 0 : allParsedQuestions.size();
	}
	...
	public SmacParser(OutputStream s, boolean tagListing, boolean pedanticParsing) throws Exception
	{
		this.ui = this;
	...
	public static void main(String[] args)
	{
		...
		try
		{
			parser = new SmacParser(System.out, tagListing, pedanticParsing);
